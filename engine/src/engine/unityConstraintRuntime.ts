...
type ResolvedRuntimeConstraint = {
  source: RuntimeConstraintSource;
  type: string;
  ownerPath: string | null;
  ownerName: string | null;
  owner: { node: THREE.Object3D | null; reason: string };
  worldUpObject: THREE.Object3D | null;
  sources: ResolvedConstraintSource[];
  sourceDebug: RuntimeConstraintDebug["constraints"][number]["sources"];
};
...
export class UnityConstraintRuntime {
  private readonly constraints: ResolvedRuntimeConstraint[];
  ...
  update(): RuntimeConstraintDebug {
    ...
    const entries = this.constraints.map(applyResolvedRuntimeConstraint);
    this.graph.root.updateMatrixWorld(true);
    return makeRuntimeConstraintDebug(this.setup, entries);
  }
}
...
export function applyUnityRuntimeConstraints(
  ...
  return makeRuntimeConstraintDebug(setup, entries);
}

function makeRuntimeConstraintDebug(
  setup: RuntimeConstraintSetupSource,
  entries: RuntimeConstraintDebug["constraints"]
): RuntimeConstraintDebug {
...
function applyResolvedRuntimeConstraint(
  resolved: ResolvedRuntimeConstraint
